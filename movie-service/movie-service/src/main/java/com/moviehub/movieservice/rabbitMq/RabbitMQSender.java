...
public class RabbitMQSender {
    ...
    public void send(Movie movie) throws JsonProcessingException {
        logger.info("Storing movie (routing key = success)");
        rabbitTemplate.setExchange(directExchange);
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        objectMapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
        rabbitTemplate.convertAndSend(routingKeySuccess, objectMapper.writeValueAsString(movie.getId()));
